...
def get_fitness_batch(
    ...
    The index mapping is the same as in reward.py:
    0=region, 1=path_length, 2=interactive,
    3=hazard, 4=collectable, 5+=none

    Fitness does not use placement quality; it evaluates only amount attainment.
    """
    fitness_funcs = [
        lambda cond, curr_map: get_region_fitness(curr_map, cond[0]) * FitnessWeight.REGION,  # 0: region
        lambda cond, curr_map: get_path_length_fitness(curr_map, cond[1]) * FitnessWeight.PATH_LENGTH,  # 1: path_length
        lambda cond, curr_map: get_multigame_amount_fitness(curr_map, cond[2], tile_name="interactive") * FitnessWeight.MONSTER,  # 2: interactive
        lambda cond, curr_map: get_multigame_amount_fitness(curr_map, cond[3], tile_name="hazard") * FitnessWeight.MONSTER,  # 3: hazard
        lambda cond, curr_map: get_multigame_amount_fitness(curr_map, cond[4], tile_name="collectable") * FitnessWeight.MONSTER,  # 4: collectable
        lambda cond, curr_map: 0.0,  # 5+: no-op
    ]
