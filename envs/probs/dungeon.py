...
class DungeonProblem(Problem):
    ...
    stat_weights = {
        DungeonMetrics.N_REGIONS: 0,
        DungeonMetrics.N_PLAYERS: 0,
        DungeonMetrics.N_ENEMIES: 0,
        DungeonMetrics.N_KEYS: 0,
        DungeonMetrics.N_DOORS: 0,
        DungeonMetrics.NEAREST_ENEMY: 0,
        DungeonMetrics.PATH_LENGTH: 0,
    }
    ...
    def get_curr_stats(self, env_map: chex.Array):
        ...
        # n_enemies = jnp.sum(
        #     (env_map == DungeonTiles.BAT)
        #     | (env_map == DungeonTiles.SCORPION)
        #     | (env_map == DungeonTiles.SPIDER)
        # )
        # n_regions, _ = calc_n_regions(
        ...
        #         src=DungeonTiles.PLAYER,
        #         trg=DungeonTiles.KEY,
        #     ),
        #     lambda: (
        #         0.0,
        ...
        #     (self.passable_tiles, jnp.array([DungeonTiles.DOOR]))
        # )
        ...
        #         src=DungeonTiles.KEY,
        #         trg=DungeonTiles.DOOR,
        #     ),
        #     lambda: (
        #         0.0,
        #         jnp.zeros(env_map.shape, dtype=jnp.float32),
        ...
        #     (self.passable_tiles, jnp.array([DungeonTiles.BAT]))
        # )
        ...
        state = DungeonState(
            stats=stats,
            player_key_flood_count=0,
            key_door_flood_count=0,
            player_enemy_flood_count=0,
            enemy_xy=0,
            k_xy=0,
            d_xy=0,
            ctrl_trgs=None,
    ...
    def init_graphics(self):
        self.graphics = {
            ...
            DungeonTiles.WALL: Image.open(f"{__location__}/tile_ims/solid.png").convert(
                "RGBA"
            ),
            DungeonTiles.BORDER: Image.open(
                f"{__location__}/tile_ims/solid.png"
            ).convert("RGBA"),
            DungeonTiles.KEY: Image.open(f"{__location__}/tile_ims/key.png").convert(
                "RGBA"
            ),
            DungeonTiles.DOOR: Image.open(f"{__location__}/tile_ims/door.png").convert(
                "RGBA"
            ),
            DungeonTiles.PLAYER: Image.open(
                f"{__location__}/tile_ims/player.png"
            ).convert("RGBA"),
            DungeonTiles.BAT: Image.open(f"{__location__}/tile_ims/bat.png").convert(
                "RGBA"
            ),
            ...
            len(DungeonTiles): Image.open(
                f"{__location__}/tile_ims/path_g.png"
            ).convert("RGBA"),
            len(DungeonTiles) + 1: Image.open(
