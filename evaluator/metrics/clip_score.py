...
================================
CLIPScore metric for HuggingFace CLIP/SigLIP text-image similarity.

Input: LevelBundle.text (instruction) + LevelBundle.image (rendered RGB)
Similarity: cosine similarity between text_i and image_j, symmetrized as (ti + tj) / 2
"""
...
class CLIPScoreMetric(BaseMetricEvaluator):
    ...
    Symmetrize text-image cosine similarities and return a similarity matrix.

    Parameters
    ----------
    model_name : str
        HuggingFace model ID.
        Flax backend: "openai/clip-vit-large-patch14" (default)
                     "openai/clip-vit-large-patch14-336"
        Torch backend: "laion/CLIP-ViT-H-14-laion2B-s32B-b79K"
                      "google/siglip-so400m-patch14-384"
    backend : {"flax", "torch"} | None
        Infer automatically from model_name when None.
    """
    ...
    def similarity_matrix(self, bundles: List[LevelBundle]) -> np.ndarray:
        """
        Symmetric (N, N) CLIPScore matrix.
