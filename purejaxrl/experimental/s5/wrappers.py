...
class FlattenObservationWrapper(GymnaxWrapper):
    ...
    def reset(
        ...
        if isinstance(obs, PCGRLObs):
            ...
            obs = jnp.concatenate(
                [obs.map_obs, obs.flat_obs[: len(self.prob.ctrl_metrics)]]
            )
        else:
...
class LogWrapper(GymnaxWrapper):
    ...
    def step(
        ...
        state = LogEnvState(
            env_state=env_state,
            #
            episode_returns=new_episode_return * (1 - prev_done),
            episode_lengths=new_episode_length * (1 - done),
            returned_episode_returns=state.returned_episode_returns * (1 - prev_done)
            + new_episode_return * prev_done,
...
class LLMRewardWrapper(GymnaxWrapper):
    ...
    def step(
        ...
        prev_array = (state.prob_state.stats - metrics_boundary[:, 0]) / (
            metrics_boundary[:, 1] - metrics_boundary[:, 0]
        )
        curr_array = (env_state.prob_state.stats - metrics_boundary[:, 0]) / (
            metrics_boundary[:, 1] - metrics_boundary[:, 0]
        )
    ...
    def extract_lc_metrics(self, metrics_enum, prefix="LC_R"):
        ...
        if self.lc_features is None:
            self.lc_features = [
                metric for metric in metrics_enum if metric.name.startswith(prefix)
            ]
...
class LLMRewardWrapperDebug(GymnaxWrapper):
    ...
    def step(
        ...
        for metric in metrics_enum:
            jax.debug.print(
                f"Metric name: {metric.name}, value = {{}}",
                env_state.prob_state.stats[metric.value],
            )
...
class LossLogWrapper(LogWrapper):
    ...
    def step(
        ...
        loss = get_loss(
            log_env_state.env_state.prob_state.stats,
