...
class Dungeon2Problem(Problem):
    ...
    def get_path_coords(self, env_map: chex.Array, prob_state: Dungeon2State):
        ...
        pk_coords = get_path_coords(
            pk_flood_count, max_path_len=self.max_path_len, coord1=k_xy
        )
        kd_coords = get_path_coords(
            kd_flood_count, max_path_len=self.max_path_len, coord1=d_xy
        )
        dt_coords = get_path_coords(
            dt_flood_count, max_path_len=self.max_path_len, coord1=t_xy
        )
        pe_coords = get_path_coords(
            pe_flood_count, max_path_len=self.max_path_len, coord1=e_xy
        )
    ...
    def get_curr_stats(self, env_map: chex.Array):
        ...
        n_enemies = jnp.sum(
            (env_map == Dungeon2Tiles.BAT)
            | (env_map == Dungeon2Tiles.SCORPION)
            | (env_map == Dungeon2Tiles.SPIDER)
        )
        n_regions, regions_flood = calc_n_regions(
            self.flood_regions_net, env_map, self.passable_tiles
        )

        def is_door_on_threshold():
            padded_regions_flood = jnp.pad(
                regions_flood, ((1, 1), (1, 1)), constant_values=0
            )
            ...
            door_patch = jax.lax.dynamic_slice(
                padded_regions_flood, (d_xy[0] - 1, d_xy[1] - 1), (3, 3)
            )
            adj_to_door = door_patch * adj_mask
            return (
                    jnp.clip(jnp.unique(adj_to_door, size=4, fill_value=0), 0, 1).sum() >= 2
            )
        ...
        def is_treasure_in_room():
            p_xy = jnp.argwhere(env_map == Dungeon2Tiles.PLAYER, size=1, fill_value=-1)[
                0
            ]
            t_xy = jnp.argwhere(
                env_map == Dungeon2Tiles.TREASURE, size=1, fill_value=-1
            )[0]
        ...
        treasure_in_room = jax.lax.cond(
            jnp.logical_and(
                n_treasures == 1, jnp.logical_and(n_regions > 1, n_players == 1)
            ),
        ...
        pk_path_length, pk_flood_count, k_xy = jax.lax.cond(
            ...
            lambda: calc_path_length(
                ...
                src=Dungeon2Tiles.PLAYER,
                trg=Dungeon2Tiles.KEY,
            ),
            lambda: (
                0.0,
                jnp.zeros(env_map.shape, dtype=jnp.float32),
                jnp.full(2, dtype=jnp.int32, fill_value=-1),
            ),
        )
        kd_passable_tiles = jnp.concatenate(
            (self.passable_tiles, jnp.array([Dungeon2Tiles.DOOR]))
        )
        kd_path_length, kd_flood_count, d_xy = jax.lax.cond(
            ...
            lambda: calc_path_length(
                ...
                src=Dungeon2Tiles.KEY,
                trg=Dungeon2Tiles.DOOR,
            ),
            lambda: (
                0.0,
                jnp.zeros(env_map.shape, dtype=jnp.float32),
        ...
        dt_passable_tiles = jnp.concatenate(
            (kd_passable_tiles, jnp.array([Dungeon2Tiles.TREASURE]))
        )
        dt_path_length, dt_flood_count, t_xy = jax.lax.cond(
            ...
            lambda: calc_path_length(
                ...
                src=Dungeon2Tiles.DOOR,
                trg=Dungeon2Tiles.TREASURE,
            ),
            lambda: (
                0.0,
                jnp.zeros(env_map.shape, dtype=jnp.float32),
        ...
        pe_passable_tiles = jnp.concatenate(
            (self.passable_tiles, jnp.array([Dungeon2Tiles.BAT]))
        )
    ...
    def init_graphics(self):
        self.graphics = {
            ...
            Dungeon2Tiles.KEY: Image.open(f"{__location__}/tile_ims/key.png").convert(
                "RGBA"
            ),
            Dungeon2Tiles.DOOR: Image.open(f"{__location__}/tile_ims/door.png").convert(
                "RGBA"
            ),
            Dungeon2Tiles.PLAYER: Image.open(
                f"{__location__}/tile_ims/player.png"
            ).convert("RGBA"),
            Dungeon2Tiles.BAT: Image.open(f"{__location__}/tile_ims/bat.png").convert(
                "RGBA"
            ),
            ...
            len(Dungeon2Tiles): Image.open(
                f"{__location__}/tile_ims/path_g.png"
            ).convert("RGBA"),
            len(Dungeon2Tiles) + 1: Image.open(
                f"{__location__}/tile_ims/path_purple.png"
            ).convert("RGBA"),
        }
