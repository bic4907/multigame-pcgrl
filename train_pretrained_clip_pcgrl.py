...
from conf.config import Pretrained_CLIP_PCGRLConfig
...
# ── Pretrained_CLIP_PCGRL obs 주입: CLIP embedding → nlp_obs ───────────────────────────────
...
def main(config: Pretrained_CLIP_PCGRLConfig):
    main_entry(config, inject_obs_fn=inject_pretrained_clip_pcgrl_obs)
