...
class TestVIPCGRLWithCLIPCheckpoint:
    ...
    def test_vipcgrl_loads_clip_ckpt_and_exits_zero(self, clip_ckpt_dir, tmp_base):
    ...
    def test_vipcgrl_stdout_contains_encoder_loaded_log(self, clip_ckpt_dir, tmp_base):
        """학습 로그에 인코더 체크포인트 로딩 성공 메시지가 포함되어야 한다."""

        hydra_run_dir = os.path.join(tmp_base, "hydra_vipcgrl_log")
        ...
        result = subprocess.run(
            [
                ...
                "render_freq=-1",
                "eval_freq=-1",
                "exp_name=test_vipcgrl_ckpt_log",
                f"encoder.ckpt_path={clip_ckpt_dir}",
                f"hydra.run.dir={hydra_run_dir}",
            ],
            cwd=_ROOT,
