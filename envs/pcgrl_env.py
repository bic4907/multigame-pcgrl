...
from envs.utils import Tiles, triangle_wave
...
def gen_static_tiles(rng, static_tile_prob, n_freezies, map_shape):
    ...
    def gen_freezies(rng):
        def gen_freezie(rng):
            ...
            r_tri = triangle_wave(
                jnp.arange(map_shape[1]) / map_shape[1], x_peak=r_loc, period=2
            )
            c_tri = triangle_wave(
                jnp.arange(map_shape[0]) / map_shape[0], x_peak=c_loc, period=2
            )
            rc_tris = jnp.stack((r_tri, c_tri))
            maxval = jnp.max(rc_tris)
            minval = jnp.min(rc_tris)
            rc_cutoff = jax.random.uniform(
                rng_, shape=(2,), minval=minval * 1.5, maxval=maxval
            )
...
class PCGRLEnv(Environment):
    ...
    def __init__(self, env_params: PCGRLEnvParams):
        (
            ...
            env_params.map_shape,
            env_params.act_shape,
            env_params.rf_shape,
            env_params.problem,
            env_params.representation,
            env_params.static_tile_prob,
            env_params.n_freezies,
            env_params.n_agents,
        )
        ...
        self.nlp_input_dim = env_params.nlp_input_dim
        self.vec_input_dim = env_params.vec_input_dim
        self.clip_input_channel = env_params.clip_input_channel

        prob_cls = PROB_CLASSES[problem]

        env_kwargs = dict()
        self.prob: Problem = prob_cls(
            map_shape=map_shape,
            ctrl_metrics=env_params.ctrl_metrics,
        ...
        map_data = self.prob.gen_init_map(
            rng,
            randomize_map_shape=self.randomize_map_shape,
    ...
    def reset_env(
        self, rng, env_params: PCGRLEnvParams, queued_state: QueuedState
    ) -> Tuple[chex.Array, PCGRLEnvState]:
        queued_map_data = MapData(
            env_map=queued_state.map,
            actual_map_shape=jnp.array(self.map_shape),
            static_map=queued_state.frz_map,
        )

        map_data = jax.lax.cond(
            queued_state.has_queued_map,
            lambda: queued_map_data,
            lambda: self.prob.gen_init_map(
                rng,
                randomize_map_shape=self.randomize_map_shape,
                empty_start=self.empty_start,
                pinpoints=self.pinpoints,
        ...
        if self.pinpoints:
            pinpoint_tiles = jnp.array(
                [
                    tile
                    for tile, num in zip(self.tile_enum, self.prob.tile_nums)
                    if num > 0
                ]
            )
        ...
        # if the problem is
        frz_map = frz_map | map_data.static_map
    ...
    def step_env(self, rng, env_state: PCGRLEnvState, action, env_params, agent_id):
        ...
        reward, prob_state = jax.lax.cond(
            ...
            lambda _: (0.0, env_state.prob_state),
            env_map,
        )
        obs = self.get_obs(
            env_map=env_map,
            frz_map=env_state.static_map,
            rep_state=rep_state,
            prob_state=prob_state,
        )
        ...
        env_state = PCGRLEnvState(
            ...
            queued_state=env_state.queued_state,
        )
    ...
    def is_terminal(self, state: PCGRLEnvState, params: PCGRLEnvParams) -> bool:
        """Check whether state is terminal."""
            ...
            state.step_idx >= (self.rep.max_steps - 1),
            jnp.logical_and(
    ...
    def render(self, env_state: PCGRLEnvState):
        path_coords_tpl = self.prob.get_path_coords(
            env_map=env_state.env_map, prob_state=env_state.prob_state
        )

        return render_map(self, env_state, path_coords_tpl)

    def render_env_map(self, env_map: jnp.array):
        return render_map(self, env_map, None, is_env_map=True, draw_path=False)
    ...
    def sample_action(self, rng):
        ...
        return jax.random.randint(rng, act_window_shape, 0, self.rep.action_space.n)[
            None, ...
        ]
...
def render_map(
    ...
    if draw_path:
        lvl_img = env.prob.draw_path(
            lvl_img=lvl_img,
            env_map=env_map,
            path_coords_tpl=path_coords_tpl,
    ...
    if draw_path and hasattr(env_state.rep_state, "pos"):
...
def render_stats_jax(env: PCGRLEnv, env_state: PCGRLEnvState, lvl_img: chex.Array):
    ...
    lvl_img_text = jnp.zeros(
        (row_height * n_rows, full_width * tile_size, 4), dtype=jnp.uint8
    )
    ...
    for i, s in enumerate(env_state.prob_state.stats):
        ...
        text = (
            "trg: "
            + "".join([" " for _ in range(max(0, len(metric_name) + 2 - 5))])
            + f"{trg}"
        )
        char_arrs = [env.prob.ascii_chars_to_ims[c] for c in text]
        text_im = jnp.concatenate(char_arrs, axis=1)
    ...
    text_im_rows = [
        jnp.pad(r, ((0, 0), (0, max_row_width - r.shape[1]), (0, 0)))
        for r in text_im_rows
    ]
    lvl_img_text = jnp.concatenate(text_im_rows, axis=0)

    lvl_img = jnp.pad(
        lvl_img, ((0, 0), (0, max(0, max_row_width - lvl_img.shape[1])), (0, 0))
    )
